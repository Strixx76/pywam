...
class Speaker:
    ...
    async def cmd_pause(self) -> None:
        ...
        if self.attribute._submode == "url" or self.attribute.app_name == "Unknown":
            ...
            # to stop.
            pause = await self.client.request(
                api_call.set_uic_playback_control("pause")
            )
            if pause.success:
    ...
    async def play_preset(self, preset: MediaPreset) -> None:
        ...
        result = await self.client.request(
            api_call.set_play_preset(preset_type, preset_index)
        )
    ...
    async def set_volume(self, volume: int) -> ApiResponse:
        ...
        volume = self.device.encode_volume(volume, self.attribute._stepofvolume)
    ...
    async def get_volume(self) -> int:
        ...
        if volume := response.get_key("volume"):
            return self.device.decode_volume(int(volume), self.attribute._stepofvolume)
        raise ApiCallError(f"({self.ip}) API call failed")
    ...
    async def group(
        ...
        if (
            self.attribute.mac is None
            or self.attribute.name is None
            or self.attribute.is_grouped is None
        ):
        ...
        if self.attribute.is_slave:
            raise PywamError(
                f"({self.ip}) This speaker is already a slave in another group"
            )
        ...
        for speaker in speakers_to_remove:
            _LOGGER.info(
                '(%s) Adding "%s" to be removed', self.ip, speaker.attribute.name
            )
        ...
        for speaker in slaves_after:
            ...
            _LOGGER.info(
                "(%s) %s will soon be called and set to slave",
                self.ip,
